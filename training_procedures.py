...
def fl_train(args, params):
    ...
    global_rewards, global_data, global_high_priority = [], [], []
        ...
        for i in range(n_workers):
            runners[i].env.learning_channel_model = \
                SLAL(buffer_size=10000, channel_param=params['ch_param'], hidden_layers=ch_hidden_layers,
                     city=params['city'], device=args.device, args=args)
    ...
    if args.load_model:
        try:
            # Load global state tracker
            global_data = list(np.load(save_path + '/global_data.npy'))
            global_high_priority = list(np.load(save_path + '/global_high_priority.npy'))
            global_rewards = list(np.load(save_path + '/global_rewards.npy'))
            start_k = len(global_data) - 1 # Accounts for the initial evaluation 
            
            # Load individual worker trackers
            for i in range(n_workers):
                runners[i].episode_data = list(np.load(save_path + f'/episode_data_{i}.npy'))
                runners[i].episode_rewards = list(np.load(save_path + f'/episode_rewards_{i}.npy'))
                runners[i].episode_high_priority = list(np.load(save_path + f'/episode_high_priority_{i}.npy'))
                runners[i].episode_priority_data = list(np.load(save_path + f'/episode_priority_data_{i}.npy', allow_pickle=True))
                
                data_evl_list[i] = runners[i].episode_data[:]
                reward_evl_list[i] = runners[i].episode_rewards[:]
                
                episode_steps[i] = len(runners[i].episode_data) * args.evaluate_cycle
                evaluate_steps[i] = len(runners[i].episode_data) - 1
                train_steps[i] = episode_steps[i] * args.train_steps
                best_episode_data[i] = max(data_evl_list[i]) if data_evl_list[i] else 0
                
                # Restore epsilon and buffer for each worker
                runners[i].rolloutWorker.load_epsilon(suffix=str(i))
                runners[i].buffer.load(suffix=str(i))
                if runners[i].env.learning_channel_model:
                     runners[i].env.learning_channel_model.load(suffix=str(i))

            print(f"Resuming federated training from period {start_k} (episode {episode_steps[0]})...")
        except Exception as e:
            print(f"Could not load previous federated data smoothly: {e}. Starting fresh logic.")
            start_k = 0
            global_data_evl, global_high_priority_evl, global_reward_evl, _, _, _, _ = runners[0].evaluate()
            global_data.append(global_data_evl)
            global_high_priority.append(global_high_priority_evl)
    ...
    for k in range(start_k, int(total_episodes / aggregation_period)):
        ...
        print('Global high priority data : ', global_high_priority_evl)
        ...
        np.save(save_path + '/global_high_priority', global_high_priority)
