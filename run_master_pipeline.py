...
def main():
    ...
    for i, model in enumerate(models):
        ...
        try:
            ...
            print("\n[!] Pipeline interrupted by user in Kaggle/Terminal.")
            print("    Waiting for the current model to safely save its massive replay buffers to disk (do NOT press stop again)...")
            try:
                process.wait(timeout=120)
            except subprocess.TimeoutExpired:
                print("    Save operation took longer than 120 seconds. Forcing termination.")
                process.terminate()
            sys.exit(1)
