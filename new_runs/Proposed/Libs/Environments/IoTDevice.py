...
class IoTDevice:
    ...
    def __init__(self, position=np.array([0, 0, 0]), color='blue', data=10000.0):
        ...
        self.deadline = float('inf') # default deadline
...
class DeviceList:
    ...
    def __init__(self, position, color, data, num_device, priorities=None, deadlines=None):
        if priorities is None:
            priorities = [1] * num_device
        if deadlines is None:
            deadlines = [float('inf')] * num_device
        self.devices = [IoTDevice(position[k], color[k], data[k])
                        for k in range(num_device)]
        for k, device in enumerate(self.devices):
            device.priority = priorities[k]
            device.deadline = deadlines[k]
