...
class SLAL:
    def __init__(self, buffer_size, channel_param=None, hidden_layers=None, city=None, device='cpu', args=None):
        self.args = args # Store args for pathing
        ...
        # Persistence path
        self.save_path = self.args.result_dir + '/' + self.args.alg + '/' + self.args.map + self.args.tag if hasattr(self, 'args') else None
    ...
    def save(self, suffix=''):
        ...
        # Save model weights
        m_filename = '/channel_model.pkl' if suffix == '' else f'/channel_model_{suffix}.pkl'
        torch.save(self.model.state_dict(), self.save_path + m_filename)
        
        # Save localization buffer (convert deques to lists for pickle)
        b_filename = '/loc_buffer.pkl' if suffix == '' else f'/loc_buffer_{suffix}.pkl'
        serializable_buff = {k: list(v) for k, v in self.localization_buff.items()}
        with open(self.save_path + b_filename, 'wb') as f:
            pickle.dump(serializable_buff, f)
            
        print(f'Successfully saved SLAL state to {self.save_path}')
    ...
    def load(self, suffix=''):
        ...
        # Load model weights
        m_filename = '/channel_model.pkl' if suffix == '' else f'/channel_model_{suffix}.pkl'
        m_path = self.save_path + m_filename
        if os.path.exists(m_path):
            self.model.load_state_dict(torch.load(m_path, map_location=self.device))
            print(f'Successfully loaded SLAL model from {m_path}')
            
        # Load localization buffer
        b_filename = '/loc_buffer.pkl' if suffix == '' else f'/loc_buffer_{suffix}.pkl'
        b_path = self.save_path + b_filename
        if os.path.exists(b_path):
            with open(b_path, 'rb') as f:
                loaded_buff = pickle.load(f)
                self.localization_buff = {k: deque(v, maxlen=1000) for k, v in loaded_buff.items()}
            print(f'Successfully loaded SLAL buffer from {b_path}')
