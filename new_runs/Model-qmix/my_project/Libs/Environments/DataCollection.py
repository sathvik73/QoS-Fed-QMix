...
class DataCollection(EnvironmentBase):
    def __init__(self,
        ...
        # Priority-Aware FedQMIX Modification: Assign priorities
        # Priority-Aware FedQMIX Modification: Assign priorities
        # Always generate the SAME variable priorities for fair comparison
        # We rely on the random seed being set in main.py/training_procedures.py for consistency across runs
        np.random.seed(args.seed if hasattr(args, 'seed') else 42) 
        self.device_priorities = np.random.randint(1, 11, size=self.n_devices)
        
        # Deadline Awareness: Assign random deadlines
        # Episode limit is roughly battery * 2 (~110). Setting deadlines in range [50, 150]
        # Some devices will expire mid-episode, some will last.
        self.device_deadlines = np.random.randint(50, 151, size=self.n_devices)
        ...
        self.device_list = DeviceList(position=params['device_position'].copy(), color=params['color'].copy(),
                                      ...
                                      priorities=self.device_priorities,
                                      deadlines=self.device_deadlines)
        ...
        self.obs_device_feats_size = (self.n_devices, 9) # Added deadline feature
    ...
    def reset(self, model=False):
        ...
        self.total_collected_data = 0
        self.total_high_priority_collected = 0
        self.total_collected_per_priority = defaultdict(float)
        self.last_action = np.zeros((self.n_agents, self.n_actions))
    ...
    def step(self, actions, model=False):
        ...
        terminated = False
        info = []
        self.step_weighted_reward = 0
        # self.high_priority_collected = 0 # Not needed as instance var if we accumulate to total directly

        # Deadline Check: Expire data if deadline passed
        for device in self.device_list.devices:
            if self._episode_steps > device.deadline:
                device.remaining_data = 0
        ...
        for a_id, action in enumerate(actions):
            ...
            if agent.battery > 0 and agent.done is False:
                ...
                if device_id != -1:
                    ...
                    # Ensure collected_data is a scalar
                    collected_data = float(np.sum(collected_data))
                    
                    agent.collected_device = np.array([device_id])
                    agent_collected_data.append(collected_data)
                    agent.collected_data += collected_data # Track individual agent load
                    
                    # Priority Calculation
                    # Priority Calculation
                    real_priority = device.priority
                    
                    if self.args.priority:
                        # Proposed: Reward reflects true priority
                        reward_priority = real_priority
                    else:
                        # Baseline: Agent thinks everything is Priority 1
                        reward_priority = 1

                    self.step_weighted_reward += collected_data * reward_priority
                    
                    # Log based on REAL priority to see what it actually collected
                    self.total_collected_per_priority[real_priority] += collected_data
                    if device.priority == 10:
                        self.total_high_priority_collected += collected_data
        ...
        # Priority-Aware Reward Calculation
        # Use weighted reward instead of raw data reward
        # Priority-Aware Reward Calculation
        # Use weighted reward instead of raw data reward
        # Load Balancing: Penalize variance in agent loads
        agent_loads = [agent.collected_data for agent in self.agents]
        load_imbalance = np.std(agent_loads) if len(agent_loads) > 1 else 0
        load_balance_lambda = 0.1 # Tune this coefficient
        
        reward = (self.step_weighted_reward / self.reward_scale) - np.sum(movement_reward) - (load_balance_lambda * load_imbalance / self.reward_scale)
    ...
    def get_obs_agent(self, agent_id):
        ...
        if agent.battery > 0 and agent.done is False:
            ...
            for d_id, device in enumerate(self.device_list.devices):
                ...
                # Priority feature (normalized)
                if self.args.priority:
                     device_feats[d_id, 7] = device.priority / 10.0
                else:
                     # Baseline: Obs sees constant priority (e.g., 1/10) matching its reward perception
                     device_feats[d_id, 7] = 0.1

                # Deadline feature (normalized by episode_limit which is max time)
                # Helps agent know how urgent the data is
                time_remaining = max(0, device.deadline - self._episode_steps)
                device_feats[d_id, 8] = time_remaining / self.episode_limit
    ...
    def get_state_dict(self):
        ...
        for de_id, device in enumerate(self.device_list.devices):
            ...
            # Priority state
            if self.args.priority:
                device_state[de_id, 3] = device.priority / 10.0
            else:
                 # Baseline: State sees constant priority
                device_state[de_id, 3] = 0.1
            
            # Deadline state
            time_remaining = max(0, device.deadline - self._episode_steps)
            device_state[de_id, 4] = time_remaining / self.episode_limit
