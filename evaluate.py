...
if __name__ == '__main__':
    ...
    # load the model, you can also choose the best model to evaluate by changing Nonebest to best in the file name
    if args.alg == 'qmix':
        if os.path.exists(model_dir + '/Nonebest_rnn_net_params.pkl'):
            path_rnn = model_dir + '/Nonebest_rnn_net_params.pkl'
            path_qmix = model_dir + '/Nonebest_qmix_net_params.pkl'
            map_location = device
    ...
    total_collected_data, _, episode_rewards, uav_trjs, device_idx, action_record, _ = runner.evaluate()
    ...
    for i in range(args.n_agents):
        ...
        for j in range(len(uav_trjs[i])):
            ...
            if isinstance(val, (np.ndarray, np.generic)):
                ...
                dev_id = val
            device_color_list[i].append(ColorMap[int(dev_id)])
    ...
    uav_start_pose = np.resize(params['start_pose'][:args.n_agents], (args.n_agents, 3))
    uav_terminal_pose = np.resize(params['end_pose'][:args.n_agents], (args.n_agents, 3))
