...
# Generate a pool of start/end poses for up to 50 agents
# to avoid IndexError when n_agents > 3.
...
uav_start_pose_list = []
uav_terminal_pose_list = []
# Battery budget will be handled below (it's a simple 1D array)

for i in range(n_max_agents):
    # Stack height: 65, 60, 55 repeating
    height = 65 - (i % 3) * 5 
    
    # Stagger XY slightly so they don't perfectly overlap
    # Every 3 agents, shift X by 10 meters
    offset_x = (i // 3) * 10
    
    uav_start_pose_list.append([[start_base[0] + offset_x, start_base[1], height]])
    uav_terminal_pose_list.append([[end_base[0] + offset_x, end_base[1], height]])

uav_start_pose = np.array(uav_start_pose_list)
uav_terminal_pose = np.array(uav_terminal_pose_list)
...
uav_battery_budget = np.full(n_max_agents, 80.0)
